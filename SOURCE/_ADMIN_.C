...
void admin_database(int *page, unsigned long *ID , LINKLIST *LIST){
    ...
    while (*page == ADMIN_DATABASE){
        ...
        admin_flush_buttons(&tag, STRUCT_LENGTH(AdminButtons), AdminButtons);
        admin_handle_buttons_event(page);
        admin_handle_database_event(LIST, &flag, page, id_list, &fp, search_str, search_needed, &selected_id); // 处理点击事件
...
void admin_modify_data(LINKLIST *LIST, FILE *fp, char *file_type, unsigned long *user_id)
{
    ...
    if(strcmp(file_type, "user_data") == 0){ // 修改用户信息
        ...
        define_admin_buttons(AdminButtons, ADMIN_MODIFY_DATA_EBIKE_INFO); // 定义按钮
    }else { // 错误
        return;
    }
    ...
    drawgraph_admin_menu(); // 绘制界面
    drawgraph_admin_database();
    if(flag == ADMIN_DATABASE_USER_INFO) drawgraph_admin_database_user_info();
    else if (flag == ADMIN_DATABASE_EBIKE_INFO) drawgraph_admin_database_ebike_info();
}
...
void drawgraph_admin_feature(int page)
{
    ...
    else if (page == ADMIN_BIKE_BROKEN)
    { // 车辆违规，车辆报废
    ...
    else if (page == ADMIN_BIKE_VIOLATION)
    { // 车辆违规
        puthz(ADMIN_INTERFACE_X1 + 20, ADMIN_INTERFACE_Y1 + 40, "学号", 16, 16, MY_WHITE);
        puthz(ADMIN_INTERFACE_X1 + 100, ADMIN_INTERFACE_Y1 + 40, "车辆车牌号", 16, 16, MY_WHITE);
        puthz(ADMIN_INTERFACE_X1 + 200, ADMIN_INTERFACE_Y1 + 40, "地点", 16, 16, MY_WHITE);
        puthz(ADMIN_INTERFACE_X1 + 300, ADMIN_INTERFACE_Y1 + 40, "时间", 16, 16, MY_WHITE);
    }
...
void drawgraph_admin_database(void)
{
    ...
    puthz(ADMIN_INTERFACE_X1 + 115, ADMIN_INTERFACE_Y1 + 10, "请选择数据", 16, 16, MY_WHITE);
...
{
    setfillstyle(SOLID_FILL, MY_LIGHTGRAY);
    bar(ADMIN_INTERFACE_X1 + 115, ADMIN_INTERFACE_Y1 + 10, ADMIN_INTERFACE_X1 + 200, ADMIN_INTERFACE_Y1 + 30);
    bar(ADMIN_INTERFACE_X1 + 20, ADMIN_INTERFACE_Y1 + 40, ADMIN_INTERFACE_X1 + 500, ADMIN_INTERFACE_Y1 + 60); // 清理画面
...
int drawgraph_admin_modify_data(FILE *fp, LINKLIST *LIST, int flag, unsigned long user_id)
{
    ...
    else if (flag == ADMIN_DATABASE_EBIKE_INFO) // 修改车辆信息
    {
        ...
        if (node->USER_DATA.anual_check != 0) // 显示年审日期
        {
        ...
        {
            outtextxy(ADMIN_LIST_DATA_INTERFACE_X1 + 80, ADMIN_MODIFY_DATA_INPUTBAR5_Y1 + 11, "0"); // 显示年审日期
        }
...
int admin_check_license(char *ebike_license){
    ...
    {
        if (ebike_license[i] < '0' || ebike_license[i] > '9') // 检查电动车通行证后六位是否为数字
        {
            return 0;
        }
    }
...
void admin_handle_manage_feature_event(LINKLIST *LIST, long ID, int *page, char *search_str, unsigned long *id_list, FILE *fp_EBIKE_INFO_read, int *mode, unsigned long *selected_id)
{
    ...
    if (mouse_press(ADMIN_FEATURE_UP_X1, ADMIN_FEATURE_UP_Y1, ADMIN_FEATURE_UP_X2, ADMIN_FEATURE_UP_Y2) == 1) // 点击上翻
    {
        *selected_id = 0; // 清除选中行
        setfillstyle(SOLID_FILL, MY_LIGHTGRAY);
        bar(ADMIN_INTERFACE_X1 + 10, ADMIN_INTERFACE_Y1 + 70, ADMIN_INTERFACE_X1 + 18, ADMIN_INTERFACE_Y1 + 310); // 清理所有选中行高亮
        admin_list_info(LIST, LIST_LIMIT, LIST_INTERVAL, id_list, fp_EBIKE_INFO_read, "ebike_manage",
                        list_mode, *mode, LIST_PAGEUP, LIST_NO_CLEAR, search_str, "ID"); // 刷新列表
    }

    if (mouse_press(ADMIN_FEATURE_DOWN_X1, ADMIN_FEATURE_DOWN_Y1, ADMIN_FEATURE_DOWN_X2, ADMIN_FEATURE_DOWN_Y2) == 1) // 点击下翻
    {
        *selected_id = 0; // 清除选中行
        setfillstyle(SOLID_FILL, MY_LIGHTGRAY);
        bar(ADMIN_INTERFACE_X1 + 10, ADMIN_INTERFACE_Y1 + 70, ADMIN_INTERFACE_X1 + 18, ADMIN_INTERFACE_Y1 + 310); // 清理所有选中行高亮
        admin_list_info(LIST, LIST_LIMIT, LIST_INTERVAL, id_list, fp_EBIKE_INFO_read, "ebike_manage",
                        list_mode, *mode, LIST_PAGEDOWN, LIST_NO_CLEAR, search_str, "ID"); // 刷新列表
    }
    ...
    if (mouse_press(ADMIN_FEATURE1_X1, ADMIN_FEATURE1_Y1, ADMIN_FEATURE1_X2, ADMIN_FEATURE1_Y2) == 1 && *selected_id != 0)
    {
        ...
        switch (*page)
        {
            ...
            case ADMIN_BIKE_VIOLATION:
                ...
                break;
            case ADMIN_BIKE_ANUAL:
                temp_node->USER_DATA.anual_check = temp_info.anual_check; // 将链表中对应节点的annual加赋值为新数据
                message_admin_quick_message(LIST, ID, temp_node->USER_DATA.ID, ANNOUNCEMENT, QUICK_ANUAL); // 发送消息
                break;
            case ADMIN_BIKE_BROKEN:
                strcpy(temp_node->USER_DATA.ebike_ID, "0");
                strcpy(temp_node->USER_DATA.ebike_license, "0");
                strcpy(temp_node->USER_DATA.location, "0");
                temp_node->USER_DATA.anual_check = 0;
                temp_node->USER_DATA.violations = 0;
                                 ...
                                 FILE **fp, char *search_str, char *search_needed, unsigned long *selected_id)
{
        ...
        *flag != ADMIN_DATABASE_USER_INFO)
    {
        ...
        *flag = ADMIN_DATABASE_USER_INFO;
        strcpy(file_type, "user_data");
        strcpy(search_needed, "ID");
        if (*fp != NULL)
        {
            fclose(*fp);                        // 关闭原文件
        }
        *fp = fopen("DATA\\USER.DAT", "rb+"); // 打开新文件
        admin_list_info(LIST, LIST_LIMIT, LIST_INTERVAL, id_list, *fp, file_type, NULL, NULL, LIST_STAY, LIST_CLEAR_CONTINUE, search_str, search_needed);
        return;
    }
    /*点击车辆信息*/
        ...
        *flag != ADMIN_DATABASE_EBIKE_INFO)
    {
        ...
        *flag = ADMIN_DATABASE_EBIKE_INFO;
        ...
        admin_list_info(LIST, LIST_LIMIT, LIST_INTERVAL, id_list, NULL, file_type, NULL, NULL, LIST_STAY, LIST_CLEAR_CONTINUE, search_str, search_needed);
        return;
    }
    /*点击车辆出入信息*/
    if (mouse_press(ADMIN_FEATURE3_X1, ADMIN_FEATURE3_Y1, ADMIN_FEATURE3_X2, ADMIN_FEATURE3_Y2) == 1 &&
    ...
    if (mouse_press(ADMIN_FEATURE_UP_X1, ADMIN_FEATURE_UP_Y1, ADMIN_FEATURE_UP_X2, ADMIN_FEATURE_UP_Y2) == 1){
        if (*flag == ADMIN_DATABASE_USER_INFO)
        {
            ...
            admin_list_info(LIST, LIST_LIMIT, LIST_INTERVAL, id_list, NULL, file_type, NULL, NULL, LIST_PAGEUP, LIST_NO_CLEAR, search_str, search_needed);
        }
    ...
    if (mouse_press(ADMIN_FEATURE_DOWN_X1, ADMIN_FEATURE_DOWN_Y1, ADMIN_FEATURE_DOWN_X2, ADMIN_FEATURE_DOWN_Y2) == 1){
        if (*flag == ADMIN_DATABASE_USER_INFO)
        {
            ...
            admin_list_info(LIST, LIST_LIMIT, LIST_INTERVAL, id_list, NULL, file_type, NULL, NULL, LIST_PAGEDOWN, LIST_NO_CLEAR, search_str, search_needed);
        }
        else if (*flag == ADMIN_DATABASE_EBIKE_PASS_IN_OUT){
            admin_list_info(LIST, LIST_LIMIT, LIST_INTERVAL, id_list, *fp, file_type, NULL, NULL, LIST_PAGEDOWN, LIST_NO_CLEAR, search_str, search_needed);
        }
    ...
    if (mouse_press(ADMIN_FEATURE_SEARCH_X1, ADMIN_FEATURE_SEARCH_Y1, ADMIN_FEATURE_SEARCH_X2, ADMIN_FEATURE_SEARCH_Y2) == 1)
    {
        ...
        if (*flag == ADMIN_DATABASE_USER_INFO || *flag == ADMIN_DATABASE_EBIKE_INFO) // 若在用户信息界面点击搜索框，则进入用户信息搜索界面
        {
            strcpy(search_needed, "ID");
            admin_list_info(LIST, LIST_LIMIT, LIST_INTERVAL, id_list, *fp, file_type, NULL, NULL, LIST_STAY, LIST_CLEAR_CONTINUE, search_str, search_needed); // 搜索后刷新列表
        }
        if (*flag == ADMIN_DATABASE_EBIKE_PASS_IN_OUT) // 若在车辆信息界面点击搜索框，则进入车辆信息搜索界面
        {
            strcpy(search_needed, "ebike_ID");
            admin_list_info(LIST, LIST_LIMIT, LIST_INTERVAL, id_list, *fp, file_type, NULL, NULL, LIST_STAY, LIST_CLEAR_CONTINUE, search_str, search_needed); // 搜索后
        }
        return;
    }
    /*点击修改信息*/
...
{ // 函数将在返回时清理user_id
    static MODIFY_EBIKE_DATA_BUFFER temp = {"\0", "\0", "\0", "\0"};
    char buffer1[15];
    char buffer2[15];
    ...
    if (mouse_press(ADMIN_MODIFY_DATA_INPUTBAR6_X1, ADMIN_MODIFY_DATA_INPUTBAR6_Y1,
                    ADMIN_MODIFY_DATA_INPUTBAR6_X2, ADMIN_MODIFY_DATA_INPUTBAR6_Y2) == 1) // 点击位置框
    {
        ch_input(temp.location_buffer, ADMIN_MODIFY_DATA_INPUTBAR6_X1, ADMIN_MODIFY_DATA_INPUTBAR6_Y1, 14, MY_LIGHTGRAY, INPUTBAR_NO_CLEAR, 1);
    }
                    ...
                    ADMIN_MODIFY_DATA_SAVE_X2, ADMIN_MODIFY_DATA_SAVE_Y2) == 1) // 点击保存
    {
        ...
        linklist_write_user_data(LIST);                                                                                           // 写入用户信息到文件中
        puthz(ADMIN_MODIFY_DATA_INTERFACE_X1 + 100, ADMIN_MODIFY_DATA_INTERFACE_Y1 + 10, "成功保存修改信息！", 16, 16, MY_GREEN); // 显示保存成功
                                      ...
                                      char *search_str, char *search_needed)
{
                    ...
                    ADMIN_FEATURE1_X2, ADMIN_FEATURE1_Y2) == 1) // 点击查看信息
    {
        if (*selected_id != 0) // 未选中
        {
            ...
            {
                show_text(10, 0, "cannot find message", MY_RED); // 显示未找到信息
            }
        }
    }
...
void define_admin_buttons(ADMIN_BUTTONS AdminButtons[], int page)
{
    ...
    else if (page == ADMIN_MODIFY_DATA_EBIKE_INFO)
    {
        for (i = 19, j = 0; i < 21; i++, j++)
        {
            admin_get_buttons(&AdminButtons[j], &Examples[i]);
        ...
        admin_get_buttons(&AdminButtons[2], &Examples[23]);
        admin_get_buttons(&AdminButtons[3], &Examples[24]);
        admin_get_buttons(&AdminButtons[4], &Examples[25]);
        admin_get_buttons(&AdminButtons[5], &Examples[26]);
    }
